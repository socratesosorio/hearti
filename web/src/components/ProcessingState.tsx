export function ProcessingState({ currentStep }: { currentStep: string }) {
  return (
...
function ProgressStep({ label, active }: { label: string; active: boolean }) {
  return (
    <div className={`flex items-center space-x-3 p-3 rounded-lg ${active ? 'bg-blue-50' : 'bg-slate-50'}`}>
      <div className={`h-2 w-2 rounded-full ${active ? 'bg-blue-600' : 'bg-slate-300'}`} />
      <span className={`text-sm ${active ? 'text-blue-800' : 'text-slate-500'}`}>{label}</span>
    </div>
  );
}
